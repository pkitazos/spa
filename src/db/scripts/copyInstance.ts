...
async function copyInstance() {
  ...
  await db.$transaction(async (tx) => {
    ...
    const instance = await tx.allocationInstance.findFirst({
      where: {
        allocationGroupId: oldParams.group,
        allocationSubGroupId: oldParams.subGroup,
        id: oldParams.instance,
      },
    });
    ...
    const newInstanceId = "testinstance";
    const { id, ...instanceData } = instance; // Omit autogenerated fields
    await tx.allocationInstance.create({
      data: { ...instanceData, id: newInstanceId, displayName: newDisplayName },
    });
  ...
  await db.$transaction(async (tx) => {
    ...
    for (const flag of oldFlags) {
      ...
      await tx.flag.create({
        data: { ...rest, allocationInstanceId: newInstanceId },
      });
    }
