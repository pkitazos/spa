...
export class AuthMask {
  public static async setMask(userId: string): Promise<void> {
   ...
   */
  public static async getMask(): Promise<UserDTO | undefined> {
    if (env.AUTH_MASKING === "OFF") {
      return undefined;
    }
   ...
   * */
  public static async clearMask(): Promise<void> {
    if (env.AUTH_MASKING === "OFF") {
      throw new Error("User switching is only available in development");
    }
